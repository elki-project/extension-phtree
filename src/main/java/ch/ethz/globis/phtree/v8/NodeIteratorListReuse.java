...
public class NodeIteratorListReuse<T, R> {
  ...
  ArrayList<?> resetAndRun(Node<T> node, long[] rangeMin, long[] rangeMax, 
    ...
    //TODO remove stuff
    //TODO reset valTemplate?
    //TODO avoid copying ranges?!?!
    results.clear();
    this.rangeMin = rangeMin;
    this.rangeMax = rangeMax;
    this.filter = filter;
    this.mapper = mapper;
    //System.arraycopy(rangeMin, 0, this.rangeMin, 0, DIM);
    //System.arraycopy(rangeMax, 0, this.rangeMax, 0, DIM);
    run(node);
    return results;
  }
  ...
  void run(Node<T> node) {
    ...
    long lowerLimit = 0;
    long upperLimit = 0;
    //TODO combine this with range check in checkAndApplyRange()
