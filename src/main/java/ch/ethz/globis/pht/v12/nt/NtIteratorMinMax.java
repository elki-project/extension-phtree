...
public final class NtIteratorMinMax<T> implements PhIterator64<T> {
	...
	private class PhIteratorStack {
		...
		public NtNodeIteratorMinMax<T> prepareAndPush(NtNode<T> node, long currentPrefix) {
			...
			if (ni == null)  {
				ni = new NtNodeIteratorMinMax<>(pp);
				stack[size-1] = ni;
			}
	...
	public NtIteratorMinMax(int keyBitWidth, PersistenceProvider pp) {
		this.pp = pp;
	...
	public NtIteratorMinMax<T> reset(NtNode<T> root, long min, long max) {	
		this.min = min;
		this.max = max;
		this.stack.size = 0;
